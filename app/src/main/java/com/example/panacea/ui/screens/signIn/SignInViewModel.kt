...
class SignInViewModel(private val nurseRepository: NurseRepository) : ViewModel() {
    ...
    // Data class para el resultado de las validaciones
    ...
    private fun validateSurname(surname: String): ValidationResult {
        return when {
            surname.isBlank() -> {
                ValidationResult(false, "Surname is empty")
            }
            else -> {
                ValidationResult(true, "")
            }
    ...
    private fun validatePasswords(password1: String, password2: String): ValidationResult {
        return when {
            ...
            password2.isBlank() -> {
                ValidationResult(false, "Password confirmation is empty")
            }
            password1 != password2 -> {
    ...
    // Función para convertir la fecha de nacimiento de String a Date
